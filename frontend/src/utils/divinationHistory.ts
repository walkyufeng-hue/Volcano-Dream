 ...
 * 保存历史记录（按类型分开保存）
 */
export function saveHistory(item: Omit<DivinationHistoryItem, 'id' | 'timestamp'>): DivinationHistoryItem | null {
  try {
...
export function markHistoryImageSaved(id: string, type: string): void {
  try {
    const history = getHistoryByType(type)
    const item = history.find((historyItem) => historyItem.id === id)
    if (!item) return
    item.hasImage = true
    localStorage.setItem(`${HISTORY_KEY_PREFIX}${type}`, JSON.stringify(history))
  } catch (error) {
    console.error('Failed to mark history image:', error)
  }
}
